...
# Works for JSON generated by parse_csv_to_json with structure {"key":{...},"key2":{...}}
# Usage: json_object_length "$json"
json_object_length() {
    local json="$1"
    # Fast method: count occurrences of ":{ which marks start of a top-level object value
    # This works because our generated JSON has the pattern "package_name":{"vulnerability...
    local count
    count=$(echo "$json" | grep -o '":{' | wc -l | tr -d ' ')
    echo "${count:-0}"
}
