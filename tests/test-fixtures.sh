#!/usr/bin/env bash
# The original CI scenarios, runnable locally without a network connection.
cd "$(dirname "$0")/.." || exit 1
...
POLY_REPORT=$(mktemp)
POLY_OUTPUT=$(../../script.sh --source ../test-vulnerabilities-multi.purl --export-json "$POLY_REPORT")
POLY_EXIT=$?
POLY_JSON=$(cat "$POLY_REPORT" 2>/dev/null)
rm -f "$POLY_REPORT"
if [ "$POLY_EXIT" -eq 1 ] \
...
bash test-fixtures/metadata-collision-project/test-metadata-collision.sh
if [ $? -eq 0 ]; then
...
bash test-fixtures/test-all-bugs.sh
if [ $? -eq 0 ]; then
